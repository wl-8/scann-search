...
from app.ann.faiss_indexes import IVFHNSWIndex, IVFPQIndex, IVFIndex, LSHIndex, OPQIndex, PQIndex
from app.ann.flat import FlatL2Index
from app.ann.hnsw import HNSWIndex
from app.index import constants as algo


def create_index(algorithm: str, dim: Optional[int] = None, **params: object) -> BaseANNIndex:
    """按算法名实例化索引（不构建）。
    ...
    if name == algo.ALGO_IVF:
        return IVFIndex(
            dim=dim,
            nlist=int(params.get("nlist", IVFIndex.DEFAULT_NLIST)),
            nprobe=int(params.get("nprobe", IVFIndex.DEFAULT_NPROBE)),
        )
